...
const productKeys = Object.keys(Product).filter(
  (k) => typeof Product[k as keyof typeof Product] === "number"
);
const productValues = productKeys
  .map((k) => Product[k as keyof typeof Product])
  .sort((a, b) => a - b);
...
function getDemandProductsHelper(productType: Product) {
  switch (productType) {
    ...
    case Product.Chemical:
      return [
        new Set([Product.Crop, Product.Metal, Product.Gem, Product.Fuel]),
      ];
    ...
    case Product.Machine:
      return [new Set([Product.Metal]), new Set([Product.Computer])];
    case Product.Tool:
      return [new Set([Product.Metal])];
  }
}

export const DEMAND_PRODUCTS = allProducts().map((x) =>
  getDemandProductsHelper(x)
);

export const FLAT_DEMAND_PRODUCTS = DEMAND_PRODUCTS.map((x) =>
  Immutable.Set<Product>().union(...x)
);

export const SECONDARY_PRODUCTS = Immutable.Set(
  allProducts().filter(
    (product) =>
      !RAW_MATERIALS_SET.has(product as RawMaterial) && // not raw materials
      !FLAT_DEMAND_PRODUCTS[product].subtract(RAW_MATERIALS_SET).isEmpty()
  )
...
export const OP_PRODUCTS: Product[] = allProducts().map((x) => getOpDemand(x));
